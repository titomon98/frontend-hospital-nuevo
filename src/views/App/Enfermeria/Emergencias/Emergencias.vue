<template>
  <b-container fluid>
    ...
    <b-modal id="modal-ver-servicio" size="lg" ref="modal-ver-servicio" :title="tpModal('Ver servicio')">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <template #modal-footer>
        ...
        <div class="mt-2 mb-3">
          ...
          <div v-if="puedeRevisarConsumos" class="mt-2">
            <revisar-consumos :id-cuenta="revisionConsumos.idCuenta" movimiento="SALIDAE" :rubro="rubroConsumoActivo" @finalizado="onRevisionFinalizada" />
          </div>
        </div>
    ...
    <b-modal id="modal-eliminar-expediente" ref="modal-eliminar-expediente" :title="tpModal('Eliminar expediente')">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
...
export default {
  ...
  computed: {
    ...
    // Oxígeno: dos servicios. "por hora" (precio por hora, se cobran horas+minutos) y
    // "por cilindro" (precio por cilindro, se cobra la fracción 1/4, 1/2, 3/4, 1).
    esOxigenoHora () {
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        ...
        case 'SaveQuirurgico': {
          ...
          this.formQui.movimiento = 'SALIDAE'
          this.existencias_selected_med = null
          break
        }
        case 'SaveComunes': {
    ...
    saveServicio () {
      const me = this
      // Oxígeno: la cantidad se deriva del tiempo (horas+min/60) o de la fracción de cilindro.
    ...
    // Título de modal con el nombre del paciente actual (para saber a quién se le agrega).
    tpModal (base) {
    ...
    // Fija el paciente actual buscando la fila por id en la tabla principal.
    setPaciente (id) {
    ...
    async recalcularTotalesConsumos () {
      ...
      const r = await axios.get(apiUrl + `/cuentas/getTotales/Emergencia?id=${this.idCuentaSeleccionada}`)
    ...
    async getConsumoMedicamentos (id) {
      try {
        ...
        if (response.data && response.data.id) {
          this.idCuentaSeleccionada = response.data.id
          const responseTotales = await axios.get(apiUrl + `/cuentas/getTotales/Emergencia?id=${this.idCuentaSeleccionada}`)
    ...
    onState () {
      let me = this
      axios
        .put(apiUrl + '/traslados/emergencia/hospital', {
          id: this.form.id,
        ...
        .then((response) => {
          ...
          me.showAlert()
          me.alertText = 'Se ha trasladado el paciente ' + me.form.nombres + ' exitosamente, por favor solicite asignación de habitación'
