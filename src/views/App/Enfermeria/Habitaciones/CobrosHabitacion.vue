<template>
  <b-container fluid>
    ...
    <b-modal id="modal-desactivar" ref="modal-desactivar" title="Eliminar cobro de habitación">
      <h6 class="my-4">
        ¿Desea eliminar el cobro de habitación de Q{{ habitacionToDelete.costo_base }} al paciente {{ habitacionToDelete.nombre_completo }} ?
      </h6>
      ...
      <iq-card class-name="iq-card-block iq-card-stretch iq-card-height">
        <template v-slot:headerTitle>
          <div class="center-text">
            <h3 class="card-title">HISTORIAL DE COBROS DE HABITACIÓN</h3>
          </div>
        </template>
        <template v-slot:body>
          <h4 class="card-title">COBRO DE HABITACIÓN</h4>
          <div class="row mb-3">
            <div class="col-md-4">
...
export default {
  ...
  data: () => {
    return {
      ...
      fechaDesde: null,
      fechaHasta: null,
      apiBase: apiUrl + '/detalle_cobros_habitacion/list',
      slickOptions: {
        centerMode: false,
        centerPadding: '60px',
        slidesToShow: 5,
      ...
      fields: [
        ...
        },
        {
          ...
          sortField: 'tipo_habitacion',
          title: 'Tipo de habitación',
          dataClass: 'list-item-heading'
        },
        {
          name: 'costo_base',
          sortField: 'costo_base',
          title: 'Total Cobro Habitación',
          dataClass: 'list-item-heading'
        },
        {
          name: 'ingreso',
  ...
  methods: {
    getRowClass (rowData) {
      if (!rowData.ingreso) return ''

      const partes = rowData.ingreso.split(' ')
    ...
    setData (data, area) {
      this.habitacionToDelete = data
      this.habitacionToDelete.area = area
      this.habitacionToDelete.responsable = this.currentUser.user
    },
    showAlert () {
    ...
    onState () {
      let me = this
      if (this.habitacionToDelete.area === 1) { // Eliminando medicamento
        axios
          .put(apiUrl + '/detalle_cobros_habitacion/deactivate', {
            delete: this.habitacionToDelete
          })
          .then((response) => {
