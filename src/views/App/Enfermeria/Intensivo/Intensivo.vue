      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Intensivo</h4>
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        ...
        case 'save': {
          ...
          this.formMe.movimiento = 'SALIDAI'
          this.form.selected_insumo = '0'
          break
        }
        case 'save2': {
    ...
    async getConsumoMedicamentos (id) {
      try {
        ...
        if (response.data && response.data.id) {
          this.idCuentaSeleccionada = response.data.id
          const responseTotales = await axios.get(apiUrl + `/cuentas/getTotales/Intensivo?id=${this.idCuentaSeleccionada}`)
