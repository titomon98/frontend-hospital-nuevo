<template>
  <b-container fluid>
    ...
    <b-modal id="modal-1-paquetes" ref="modal-1-paquetes" :title="tpModal('Agregar paquete a paciente')">
      <b-alert
    ...
    <b-modal id="modal-add-servicio" ref="modal-add-servicio" :title="tpModal('Añadir consumo de servicio')">
      ...
      <b-form @submit="$event.preventDefault()">
        <b-form-group label="Servicio a agregar:">
          ...
          >
            <template v-slot:option="option" v-if="puedeVerPrecios">
              {{ option.descripcion + ' - Precio: ' + option.precio }}
            </template>
            <template v-slot:option="option" v-else>
              {{ option.descripcion }}
            </template>
            <template v-slot:selected-option="option" v-if="puedeVerPrecios">
              {{ option.descripcion + ' - Precio: ' + option.precio }}
            </template>
    ...
    <b-modal id="modal-sala-operaciones" size="lg" ref="modal-sala-operaciones" :title="tpModal('Servicio sala de operaciones')">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
            ...
            <table class="table table-striped">
              <thead>
                <tr>
                  <th>Descripción</th>
                  <th>Tiempo</th>
                  <th v-if="puedeVerPrecios">Total</th>
                  <th>Agregado por</th>
              ...
              <tbody>
                ...
                <tr v-for="item in salas" :key="item.id">
                  ...
                  <td>{{ item.horas }}</td>
                  <td v-if="puedeVerPrecios">Q {{ item.total }}</td>
      ...
      <template #modal-footer>
        <div class="ml-auto"> <span v-if="puedeVerPrecios" class="mr-2">Total: Q{{ TotalAPagar }}</span></div>
    ...
    <b-modal id="modal-editar-sala" ref="modal-editar-sala" size="lg" :title="tpModal('Editar sala de operaciones')">
      ...
      <template #modal-footer>
        ...
        <div class="mt-2 mb-3">
          ...
          <div v-if="puedeRevisarConsumos" class="mt-2">
            <revisar-consumos :id-cuenta="revisionConsumos.idCuenta" movimiento="SALIDAQ" :rubro="rubroConsumoActivo" @finalizado="onRevisionFinalizada" />
          </div>
        </div>
    ...
    <b-modal id="modal_agregar" size="xl" ref="modal_agregar" :title="tpModal('Ingresar nuevo examen')">
      ...
      <template #modal-footer="{}">
        ...
        <div class="ml-auto" v-if="puedeVerPreciosExamenes">
    ...
    <b-modal id="reporteModal" :title="tpModal('Reporte de Cuenta Parcial')" size="lg">
      <div class="modal-body">
        ...
        <hr />
        <p><strong><u>Total deuda preliminar:</u> Q{{ reporte.TotalDeuda }}</strong></p>
      </div>
      <template #modal-footer>
...
export default {
  ...
  methods: {
    ...
    async recalcularTotalesConsumos () {
      if (!this.idCuentaSeleccionada) return
      const r = await axios.get(apiUrl + `/cuentas/getTotales/Quirofano?id=${this.idCuentaSeleccionada}`)
    ...
    async getConsumoMedicamentos (id) {
      try {
        ...
        if (response.data && response.data.id) {
          ...
          this.apiBaseConsumoAnestesicos = apiUrl + `/detalle_consumo_medicamentos/listAnestesicos/${response.data.id}/Quirofano`
    ...
    async guardarExamenRealizado () {
      ...
      if (this.selectedExamenes.length === 0) {
        this.alertVariant = 'danger'
        this.showAlertError()
        this.alertErrorText = 'Por favor seleccione al menos un examen'
        return
      }

      this.formExamen.id_examenes_almacenados = this.selectedExamenes.map(e => e.id)
      this.formExamen.total = this.TotalAPagar2

      try {
        await axios.post(apiUrl + '/Examenes_realizados/create', {
          form: this.formExamen
        })

        this.alertVariant = 'success'
        this.showAlert()
        this.alertText = 'Se han ingresado los exámenes de ' + this.formExamen.nombre + ' exitosamente'
        this.ver_examen_realizado(this.formExamen.id_expediente)
        this.selectedExamenes = []
        this.formExamen.total = 0
        this.TotalAPagar2 = 0
      } catch (error) {
        this.alertVariant = 'danger'
        this.showAlertError()
        this.alertErrorText = error.response?.data?.msg || 'Ha ocurrido un error al guardar los exámenes'
        console.error('Error!', error)
      }
    },
