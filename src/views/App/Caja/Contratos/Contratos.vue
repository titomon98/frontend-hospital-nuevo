<template>
  <b-container fluid>
    ...
    <b-modal id="modal-1-contract" ref="modal-1-contract" title="Agregar contrato">
      <b-alert
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Contratos</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-contract>AGREGAR NUEVO</b-button>
          </template>
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-contract'].hide()
          this.form.id = 0
          this.form.contrato = ''
          this.form.nombre = ''
          this.form.state = 1
    ...
    onSave () {
      const me = this
      axios.post(apiUrl + '/contrato/create', {
        form: me.form })
        .then((response) => {
