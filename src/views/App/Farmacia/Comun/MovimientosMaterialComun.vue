    ...
    <b-modal id="modal-1-movimiento" ref="modal-1-movimiento" title="Agregar movimiento">
      ...
      <b-form @submit="$event.preventDefault()">
        <b-form-group label="Material común:">
            <v-select
              name="comunes"
              v-model="$v.form.comun.$model"
              :state="!$v.form.comun.$error"
              :options="comunes"
              :filterable="false"
            ...
            <div v-if="$v.form.comun.$error" class="invalid-feedback-vselect">
              Debe seleccionar el material común
            </div>
          </b-form-group>
          ...
          <b-row class="ml-2">
            <b-col md="6">
              <b-form-group label="Tipo de movimiento:">
                <b-form-radio v-model="form.movimiento" value="ENTRADA" @change="changeCheck" name="customRadio1">Ingreso de material común</b-form-radio>
                <b-form-radio v-model="form.movimiento" value="SALIDA" @change="changeCheck" name="customRadio1">Salida de material común</b-form-radio>
              </b-form-group>
            </b-col>
            <b-col md="6">
      ...
      <b-form @submit="$event.preventDefault()">
        <b-form-group label="Material común:">
            <v-select
              ...
              :options="comunes"
              :filterable="false"
              placeholder="Seleccione material común"
              @search="onSearchComunes"
            >
              <template v-slot:spinner="{ loading }">
                <div v-show="loading">Cargando...</div>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Movimientos material común</h4>
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/comun_movimientos/list',
      fields: [
        {
          name: 'cantidad',
          sortField: 'cantidad',
  ...
  methods: {
      ...
      if (this.$v.$error !== true) {
        if (action === 'save') {
          if (parseInt(this.form.cantidad) < 1) {
            ...
            this.showAlertError()
          } else if (parseInt(this.form.cantidad) > this.form.comun.existencia_actual && this.form.movimiento === 'SALIDA') {
    ...
    onSave () {
      ...
      axios.post(apiUrl + '/comun_movimientos/create', {
        form: me.form,
    ...
    onUpdate () {
      ...
      axios.put(apiUrl + '/comun_movimientos/update', {
        form: me.form })
        .then((response) => {
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/comun_movimientos/deactivate', {
            id: this.form.id
          })
          .then((response) => {
        ...
        axios
          .put(apiUrl + '/comun_movimientos/activate', {
            id: this.form.id
          })
          .then((response) => {
